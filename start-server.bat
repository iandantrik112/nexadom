...
echo ================================================================================
echo.
echo [USAGE] start-server.bat [PORT]
...
REM Ask if user wants to allow access from network
echo.
...
if exist "router.php" (
    echo [INFO] Using router.php for proper MIME type handling
    echo [INFO] Starting PHP server: php -S %BIND_HOST%:%PORT% router.php
    echo.
    if "%BIND_HOST%"=="0.0.0.0" (
    ...
    php -S %BIND_HOST%:%PORT% router.php
    if errorlevel 1 (
        echo.
    ...
    echo [INFO] router.php not found, using index.php directly
    echo [INFO] Starting PHP server: php -S %BIND_HOST%:%PORT% -t . index.php
    echo.
    if "%BIND_HOST%"=="0.0.0.0" (
        echo [DEBUG] Binding to 0.0.0.0:%PORT% - server accessible from network
        echo [DEBUG] Server dapat diakses dari http://localhost:%PORT% atau http://!LOCAL_IP!:%PORT%
    )
    php -S %BIND_HOST%:%PORT% -t . index.php
    if errorlevel 1 (
        echo.
        echo [ERROR] Gagal memulai server!
        if "%BIND_HOST%"=="0.0.0.0" (
