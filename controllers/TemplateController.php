...
class TemplateController extends NexaController
{
    ...
    public function __construct($template = null, $deviceLayouts = [])
    {
        ...
        // Project root: __DIR__ = controllers/, dirname(__DIR__) = www/ (selalu sesuai lokasi kode)
    ...
    public function index($params = null): void
    {
        ...
        $realBase = realpath($this->templatesBasePath . $folder . '/');
    ...
    public function assets($params = null): void
    {
        ...
        // 1. Cookie (diset saat page render)
        $template = $_COOKIE['nexa_template'] ?? null;
        if ($template && in_array($template, $this->allowedFolders, true)) {
            // Use cookie value
        } else {
            ...
            $template = match (true) {
                $device['is_mobile'] => 'mobile',
                $device['is_tablet'] => 'tablet',
                default => 'theme',
            };
        }
        ...
        // Urutan: cookie/device template → theme (fallback) → root assets
        $templatesToTry = array_unique([$template, 'theme', 'mobile', 'tablet', 'dashboard']);
        foreach ($templatesToTry as $tryTemplate) {
            ...
            if (file_exists($tryPath) && is_file($tryPath)) {
                $fileToServe = $tryPath;
                $realBase = realpath($this->templatesBasePath . $tryTemplate . DIRECTORY_SEPARATOR . 'assets');
                break;
            }
        }

        if (!$fileToServe && file_exists($rootAssetPath) && is_file($rootAssetPath)) {
            $fileToServe = $rootAssetPath;
            $realBase = realpath($this->projectRoot . DIRECTORY_SEPARATOR . 'assets');
        }
        ...
        if (!$fileToServe || !$realBase) {
            $this->sendError(404, 'File Not Found', [
                ...
                'template_exists' => file_exists($this->templatesBasePath . $template . DIRECTORY_SEPARATOR . 'assets' . DIRECTORY_SEPARATOR . $pathNormalized),
                'root_path' => $rootAssetPath,
                'root_exists' => file_exists($rootAssetPath),
